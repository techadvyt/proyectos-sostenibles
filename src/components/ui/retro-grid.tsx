...
function getProgramInfo(gl: WebGLRenderingContext, program: WebGLProgram): ProgramInfo | null {
  ...
  if (attributeLocation < 0 || !angle || !cellSize || !containerSize || !devicePixelRatio || !lineColor || !time || !viewportSize) return null
...
function resolveLineColor(color: string, element: HTMLElement) {
  ...
  if (!context) return new Float32Array([0.5, 0.5, 0.5, 1])
...
export function RetroGrid({
  ...
  useEffect(() => {
    ...
    let currentWidth = 0, currentHeight = 0, currentDevicePixelRatio = 1
    let gl: WebGLRenderingContext | null = null
    let isVisible = true, isContextLost = false
    let lineColor = resolveLineColor(lightLineColorRef.current, container)
    let positionBuffer: WebGLBuffer | null = null
    let programInfo: ProgramInfo | null = null

    const getContext = () => {
      const nextGl = canvas.getContext("webgl", { alpha: true, antialias: true, premultipliedAlpha: true })
      if (!nextGl || !nextGl.getExtension("OES_standard_derivatives")) return null
      return nextGl
    }
    ...
    const releasePipeline = (shouldDeleteResources: boolean) => {
      ...
      positionBuffer = null; programInfo = null
      if (shouldDeleteResources) gl = null
    }

    const initializePipeline = () => {
      const nextGl = getContext()
      if (!nextGl) { releasePipeline(false); return false }
      gl = nextGl; releasePipeline(true); gl = nextGl
      ...
      positionBuffer = nextPositionBuffer; programInfo = nextProgramInfo
      return true
    }

    const updateLineColor = () => {
      const activeColor = isDarkMode() ? darkLineColorRef.current : lightLineColorRef.current
    ...
    const resizeCanvas = () => {
      ...
      canvas.style.width = `${currentWidth}px`; canvas.style.height = `${currentHeight}px`
    ...
    const stopAnimation = () => { if (animationFrameId !== null) { cancelAnimationFrame(animationFrameId); animationFrameId = null } }

    const frame = (timestamp: number) => {
      draw(timestamp)
      if (!reducedMotion.matches && isVisible) { animationFrameId = requestAnimationFrame(frame); return }
      animationFrameId = null
    }

    const syncScene = () => {
      if (isContextLost) { stopAnimation(); setIsWebGlReady(false); return }
      if (!gl || !positionBuffer || !programInfo) {
        if (!initializePipeline()) { stopAnimation(); setIsWebGlReady(false); return }
      }
      resizeCanvas()
      if (currentWidth === 0 || currentHeight === 0) { stopAnimation(); return }
      updateLineColor(); draw(performance.now()); setIsWebGlReady(true)
    ...
    const handleContextLost = (event: Event) => { event.preventDefault(); isContextLost = true; stopAnimation(); releasePipeline(false); setIsWebGlReady(false) }
    ...
    syncScene()
    return () => {
      stopAnimation(); resizeObserver.disconnect(); intersectionObserver.disconnect()
      ...
      syncSceneRef.current = null; releasePipeline(!isContextLost)
    }
  }, [])

  const gridStyles = { ...style, opacity } as CSSProperties
